...
const Index = () => {
  ...
  const addTodo = async () => {
    try {
      ...
      await axios.post(
        `http://192.168.1.3:3000/todos/${userId}`,
        todoData
      );
  ...
  const getUserTodos = async () => {
    try {
      const userId = await AsyncStorage.getItem('userId');
      const response = await axios.get(`http://192.168.1.3:3000/users/${userId}/todos`);
  ...
  const markTodoAsCompleted = async (todoId) => {
    try {
      setMarked(true);
      await axios.patch(`http://192.168.1.3:3000/todos/${todoId}/complete`);
      await getUserTodos();
  ...
  const handleDeleteTask = async (todoIdDelete) => {
    try {
      await axios.delete(`http://192.168.1.3:3000/todos/${todoIdDelete}`);
      await getUserTodos();
